    ...
    <div class="properties-body">
      ...
      <a-form v-show="selectItem.key !== ''">
        ...
        <a-form-item
          ...
          label="数据字段"
        >
          <a-input v-model="selectItem.model" placeholder="请输入" disabled/>
        </a-form-item>
        <!-- input type start -->
        <a-form-item v-if="selectItem.type === 'input'" label="输入框type">
<!--          <a-input v-model="options.type" placeholder="请输入" />-->
          <a-select default-value="text" @change="handleChange">
        ...
        <!-- input type end -->
<!--        是否作为筛选条件-->
        <a-form-item label="是否作为筛选条件" v-if="ScreeningConditionsControl">
          <a-select  @change="handleScreenChange">
...
export default {
  ...
  watch: {
    selectItem(val) {
      ...
      // this.options.isScreeningConditions = 'false'
    }
  },
  computed: {
  //  判断是否筛选条件控件的存在与否
    ScreeningConditionsControl() {
      return [
        'input',
        'number',
        // 'textarea',
  ...
  methods: {
    ...
    handleScreenChange(value) {
      this.options.isScreeningConditions = value
    },
    screeningConditions(e) {
      console.log(e)
      this.options.isScreeningConditions = e.target.value
    }
  },
  components: {
    KChangeOption,
