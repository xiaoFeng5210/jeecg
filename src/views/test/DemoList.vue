...
<!--              <j-input placeholder="请输入名称模糊查询" v-model="queryParam.createBy"></j-input>-->
...
<!--            </a>-->
    ...
    <div class="table-operator" style="margin-top: 5px">
      <a-button @click="handleAdd('add', 1)" type="primary" icon="plus">新增</a-button>
    ...
    <div>
      ...
      <a-table
        ...
        <span slot="action" slot-scope="text, record">
          <a @click="handleAdd('editor',record)">
            <a-icon type="edit"/>
            编辑
          </a>
<!--          <a-divider type="vertical"/>-->
<!--          <a @click="editDesignItem(record)"><a-icon type="setting"/> 设计表单</a>-->
          <a-divider type="vertical"/>
          <a @click="routeToCustomForm(record.id)">测试地址</a>
          <a-divider type="vertical"/>
          <a-dropdown>
...
<!--    编辑组件-->
    <DesignFormsEditorModal ref="modalForm" @ok="modalFormOk" />
    <!-- 一对多表单区域 -->
    <JeecgDemoTabsModal ref="jeecgDemoTabsModal" @ok="modalFormOk"></JeecgDemoTabsModal>

    <!--  表单设计器 -->
    <DesformEdtiorModal ref="desformEdtiorModal"
                        @ok="modalFormOk"
                        @query-list="searchQuery"
                        :row="designforms"></DesformEdtiorModal>
...
<script>
  ...
  export default {
    ...
    methods: {
      // 跳转到对应的自定义表单路由
      routeToCustomForm(id) {
        this.$router.push({
          path: '/designforms/customform',
          query: { id: id }
        })
      },
      handleAdd(type, record) {
        this.$refs['desformEdtiorModal'].show(type, record)
      },
